// Path: src/components/molecules/fing/AllocationSummary/AllocationSummary.tsx
...
const assetColors: Record<string, string> = {
  stocks: 'var(--fing-chart-1)',
  bonds: 'var(--fing-chart-4)',
  cash: 'var(--fing-chart-3)',
  commodities: 'var(--fing-chart-6)',
  crypto: 'var(--fing-chart-5)',
  real_estate: 'var(--fing-chart-7)',
  other: 'var(--fing-chart-2)',
};

function getAssetColor(assetClass: string): string {
  const key = assetClass.toLowerCase().replace(/\s+/g, '_');
  return assetColors[key] || 'var(--fing-text-tertiary)';
}
